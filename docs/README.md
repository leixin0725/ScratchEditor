...
- `2026-08-31`：修复剪贴板历史卡片拖入正文后撤销时光标丢失的问题；
  撤销现在会恢复拖入前的 UTF-16 光标，原有选区则同时恢复范围与活动端方向。
- `2026-08-31`：命令面板新增“转换为有序列表”；可转换选中行或当前行的普通无序项，
  混合选区会扩展到完整有序列表、沿用其起始编号和格式后统一编号，并保持嵌套、任务项、
  围栏代码与分隔区域边界；转换和选区外重编号可一次撤销。
