...
- `2026-08-10`：编辑器新增 `PageUp`/`PageDown` 按页纯滚动浏览、正文下方 2/3 页可滚动留白，以及输入触底/删除触顶的间歇式自动滚动；删除、剪切与命令面板删除类动作一并纳入，撤销/重做视为普通编辑共用同一套检查，见根目录 [`README.md`](../README.md#编辑快捷键) 的“编辑快捷键”。
