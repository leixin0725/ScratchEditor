...
- 标题折叠识别与语法高亮一致的 ATX 1–6 级标题，并忽略反引号围栏代码内的伪标题。可折叠标题左侧的 `v` / `>` 标记始终可见：展开时使用弱化色，折叠时使用界面强调色。折叠或展开后编辑区会在下一渲染帧立即刷新，无需改变窗口布局。折叠一个标题会隐藏其正文和下属子标题，直到下一个同级或更高级标题；“折叠所有标题”保留文档顶层大纲可见。若光标位于将被隐藏的正文，折叠后会回到最近的可见标题行首。
