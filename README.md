...
- `Ctrl+Up` / `Ctrl+Down` 按文档顺序跳到上一个/下一个标题行首；目标被折叠时自动展开必要的祖先标题。每次成功跳转会在标题实际文字范围（包含 `#`）下方显示 20% 不透明度的强调色圆角高亮，停留 400ms 后用 300ms 淡出；关闭动画时停留后立即消失。查找命中折叠正文时也会自动展开对应路径。折叠状态只属于当前编辑会话，载入新内容时重置，不写入用户配置或文件。
