...
- 可从命令面板把选中行或当前行的普通无序列表转换为有序列表；混合选区会并入完整的
  有序列表并统一重新编号。
- 针对连续中文、日文假名、韩文谚文及中英文混排优化按词移动、选择和删除。
- 根据上下文转换中文标点，并在中文与英文或标记之间整理必要的空格。
- 可直接拖动已有文本选区；拖到窗口边缘时自动滚动，整个移动可单步撤销。
- 可把一个或多个本地文件、文件夹拖入正文，在当前光标处插入绝对路径；已有选区时替换
  选区，多项按拖入顺序用单个空格连接，含空白字符的路径会用双引号包裹。路径与相邻正文
  之间不会自动补空格，整次插入可单步撤销。该功能同时适用于常驻模式和 `--wait` 文件模式。
...
- 把历史卡片拖到编辑区的指定位置；拖动时会显示复制/禁止光标和跟随指针的半透明摘要，
  插入完整内容后可用一次 `Ctrl+Z` 撤销，并恢复拖入前的光标或选区及其方向。
- 删除单项，或在确认后清空全部历史。
...
- `PageUp`/`PageDown` 按一页纯滚动浏览（光标与选区不动）；正文下方保留 2/3 页可滚动的留白区；输入导致光标碰到视口底边时自动滚动一次：段中光标行滚到视口上 1/3，段尾等效于滚到底（正文末尾下方 2/3 页留白翻出，光标停在上 1/3）；之后继续输入不再重复触发，光标自然下落，再次触底才再次触发（间歇式）。删除/剪切/撤销等使光标越过视口顶边时按严格镜像规则自动滚动：光标行滚到视口距顶 2/3 处，文档开头滚到顶部，同样间歇式触发；撤销与重做视为普通输入/删除，共用同一套自动滚动检查。内容收缩期间会在编辑前预先保持视口，避免滚动范围变化造成中间跳动。以上翻页与自动滚动在动画开关开启时使用约 160ms 的轻量平滑滚动，关闭时瞬时到位。
...
- 正文视口接受一个或多个本地文件/目录 URL 拖放，以复制动作在当前 UTF-16 光标或选区处
  插入绝对 Windows 原生路径；整批 URL 先统一校验，任一条目无效时不执行部分插入。
...
- 可把剪贴板历史卡片拖入编辑区落点，复用文本选区拖动的落点提示、边缘滚动与单步撤销。
...
停留 100ms 可展开，指针快速向左越出窗口时立即展开；指针离开触发区和面板后停留 500ms 自动
收起。也可从命令面板执行“切换剪贴板历史”，
再次执行（或按已配置快捷键）即可关闭。
...
按住历史卡片并移动超过系统拖动阈值后，可把该项完整文本插入编辑区的鼠标落点。拖动期间显示
与选区移动相同的落点光标；有效落点使用复制光标，无效区域使用禁止光标，同时显示跟随指针的
半透明卡片摘要。摘要限制在窗口内且最多三行，不遮住指针热点。靠近视口上下边缘时自动滚动；
历史面板保持展开，避免编辑区在拖动途中改变几何。成功放置后新增文本保持选中、编辑器重新获得
焦点并关闭历史面板，整个插入（包括必要的有序列表编号维护）可用一次 `Ctrl+Z` 撤销；
撤销后恢复拖入前的光标，原有选区则连同活动端方向一并恢复。放到编辑区
之外只会取消操作，不会修改正文或历史原项；未超过拖动阈值的按放仍是普通单击，也不会显示预览。
...
| 转换为有序列表 / 切换剪贴板历史 / 循环标题级别 / 切换项目列表 | 无默认快捷键 |
...
- 撤销粘贴会恢复粘贴前的光标位置；粘贴曾替换选区时，同时恢复原选区及其方向。
...
- 标题折叠识别与语法高亮一致的 ATX 1–6 级标题，并忽略反引号围栏代码内的伪标题。可折叠标题左侧的 Lucide `chevron-down` / `chevron-right` 图标始终可见：展开时使用弱化色，折叠时使用界面强调色。折叠或展开后编辑区会在下一渲染帧立即刷新，无需改变窗口布局。折叠一个标题会隐藏其正文和下属子标题，直到下一个同级或更高级标题；“折叠所有标题”保留文档顶层大纲可见。若插入光标位于将被隐藏的正文，折叠后会临时落到最近可见标题的行末；用户未主动移动或编辑时，重新展开会恢复折叠前的正文位置。
- `Ctrl+Up` / `Ctrl+Down` 按文档顺序跳到上一个/下一个标题行首；目标被折叠时自动展开必要的祖先标题。每次成功跳转会在标题实际文字范围（包含 `#`）下方显示 20% 不透明度的强调色圆角高亮，停留 400ms 后用 300ms 淡出；关闭动画时停留后立即消失。每次成功跳转后还会尝试滚动，使标题首行居于编辑区上 1/3 分点：与翻页/自动滚动共用约 160ms 轻量平滑动画（关闭动画时瞬时到位），跳转期间抑制瞬时贴边跟随以保证整段跳转是单一平滑移动，目标标题刚从折叠中展开时在布局落定后定位。查找命中折叠正文时也会自动展开对应路径。折叠状态只属于当前编辑会话，载入新内容时重置，不写入用户配置或文件。
...
- 括号、引号、行内代码与围栏代码支持自动补全，包含半角、全角及常用中文成对符号。
  ...
  中文输入法直接提交 `“` / `‘` 时同样完成配对与自动空格，随后提交 `”` / `’` 会越过已有闭符号；
  若输入法因内部状态在行尾提交孤立的 `”` / `’`（光标后允许只有空格或 Tab），则视作对应开引号
  并生成新配对；当前行存在同类未闭合开引号时仍优先完成闭合；
...
- 命令面板的“转换为有序列表”处理选区实际覆盖的整行；无选区时处理当前行。它只转换普通
  `-`、`+`、`*` 无序项，不修改任务项、普通文本或围栏代码。纯无序区域从 `1.` 开始；选区
  同时包含有序项时，会按缩进和引用层级向外找到完整列表，以最上方有序段的首项编号及
  `.` / `)` 格式为锚点，把选中的无序项接入后按文档顺序重编号。选中的无序项可桥接同一
  连续区域内格式不同的有序段；未选中的无序项、任务项、普通文本、空行和围栏形成边界，
  不同区域及嵌套层级分别编号。执行后选中全部处理范围；文本转换和选区外重编号可用一次
  `Ctrl+Z` 撤销并恢复原选区。若只有有序项，则只整理并扩选其完整列表；纯扩选不写入撤销栈。
...
界面图标使用 Lucide 路径数据；当前收录的 `chevron-down` 与 `chevron-right` 源自
Feather。完整 ISC/MIT 许可文本见 [`third_party/lucide/LICENSE`](third_party/lucide/LICENSE)。
