...
- 把历史卡片拖到编辑区的指定位置，直接插入其完整内容；插入后可用一次 `Ctrl+Z` 撤销。
- 删除单项，或在确认后清空全部历史。
...
与选区移动相同的落点光标和允许/禁止鼠标反馈，靠近视口上下边缘时自动滚动；历史面板保持展开，
避免编辑区在拖动途中改变几何。成功放置后新增文本保持选中、编辑器重新获得焦点并关闭历史面板，
整个插入（包括必要的有序列表编号维护）可用一次 `Ctrl+Z` 撤销。放到编辑区之外只会取消操作，
不会修改正文或历史原项；未超过拖动阈值的按放仍是普通单击。
