...
- `window_ui_main.cpp`：窗口界面验证，覆盖设置页、主题字体、集中配置、历史栏内外侧 hover/快速左向越界、窗口交互/配色、20 轮唤出关闭动画稳定性、闭合态窗口缩放时历史面板右边缘不外露回归、窗口放置算法单元校验（记忆恢复、混合 DPI 非主屏坐标、尺寸阶梯、锚定顺序与重叠避让）与明确排除项回归。
