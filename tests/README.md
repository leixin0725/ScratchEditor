...
- `editing_main.cpp`：编辑行为验证，覆盖 Markdown 高亮、编辑命令、标题层级折叠/导航、
  ...
  原子拒绝、撤销、无序列表转有序列表、列表修复和自动滚动，以及撤销/删除造成内容收缩时
  首帧与延迟检查之间的
  视口稳定性、查找替换和快捷键回归。
- `clipboardhistory_main.cpp`：无窗口的协调器、内存 gateway、历史集合、二进制 codec、DPAPI、
  原子存储与故障边界测试；协调器用例覆盖异步加载、自写入回声抑制、错误优先级、
  `ReadLocked` 清空与关闭刷新。
- `window_ui_main.cpp`：窗口界面验证，覆盖设置页、主题与编辑区主/fallback 字体链、九档字体粗细的默认值/应用/校验/持久化、集中配置、文件拖放区域与正文视口几何一致性及各面板/历史确认遮罩禁用状态、状态面板根级层级与高 DPI 左右边框像素强度、剪贴板历史拖拽的阈值、复制/禁止光标、半透明摘要及清理状态、Lucide 标题折叠图标的常驻显示/名称/有效性/配色/点击与滚动范围、gutter 折叠光标恢复、标题导航高亮层级/范围/时序、折叠/展开后下一帧正文像素即时消失/恢复、历史栏标题/搜索框/列表与主窗口的推挤及覆盖模式布局对齐、历史栏内外侧 hover/快速左向越界、窗口交互/配色、20 轮唤出关闭动画稳定性、动画滚动完成后撤销收缩内容时的视口稳定性、闭合态窗口缩放时历史面板右边缘不外露回归、动画开启时缩放窗口编辑区即时跟随回归（历史面板开/合两种状态）、短文本下窗口缩放与轻量关闭动画滚动条不闪现回归、窗口放置算法单元校验（记忆恢复、混合 DPI 非主屏坐标、尺寸阶梯、锚定顺序与重叠避让）与明确排除项回归。
