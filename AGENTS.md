...
7. **原子提交、部署和文档维护**：开发过程中允许自行暂存并创建本地原子提交，无需逐次申请批准；每个提交必须职责单一、可独立回退，提交信息使用简体中文并遵循 GitHub 社区通行规范。禁止自行执行 `git push`，所有远端推送均由用户完成。重要改动完成后检查并更新 `docs/README.md` 索引；已完结的功能需求移入 `docs/archive/`；部署仍按规则 12 等待用户批准。
