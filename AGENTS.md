...
7. **提交、部署和文档维护**：重要改动完成后检查并更新 `docs/README.md` 索引；已完结的功能需求移入 `docs/archive/`；用户要求提交后，若最新版尚未部署，主动询问是否需要部署。
...
11. **分支创建与命名**：新功能开发使用独立分支，命名 `NNN-<英文小写短横线名>`（如 `007-demo`），三位序号取现有分支与 `archive/` tag 的最大序号加一；从最新 `main` 创建，开发在独立 worktree 中进行，创建前确认分支名与 `archive/` tag 均未占用：
    ```powershell
    ...
    ```
12. **合并与归档**：功能完成后先在分支最终提交打轻量 tag `archive/NNN-<名>`（如 `git tag archive/007-demo 007-demo`），再在 `main` 上非快进合并，提交信息固定为 `merge: 合并<中文功能名>（NNN-<名>）`（如 `git merge --no-ff -m "merge: 合并示例功能（007-demo）" 007-demo`）；同时把已完结功能的需求文档移入 `docs/archive/` 并更新 `docs/README.md` 索引。
13. **分支删除与 tag 保留**：仅当合并完成且 `archive/` tag 已打后，先按规则 10 移除对应 worktree，再用 `git branch -d <分支名>` 删除分支；若 `-d` 拒绝删除，停止并向用户确认，禁止用 `-D` 强删。`archive/` tag 永久保留作为回溯依据。
